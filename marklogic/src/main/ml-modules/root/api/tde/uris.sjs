...
let page = xdmp.getRequestField("page"),
   pageSize = xdmp.getRequestField("pageSize"),
   finalCollectionsQuery;
...
if (template !== undefined && template !== null && !fn.empty(template)) {
  ...
  if (collections !== undefined && collections !==null && !fn.empty(collections)) {
    let uniCollections = collections
                        .filter(collection => !collection.collectionsAnd),
      andCollections = collections
                       .filter(collection => collection.collectionsAnd),
      uniCollectionsQuery = cts.collectionQuery(uniCollections),
      andCollectionsArray = andCollections
                            .map(andCollection => {
                              return andCollection.collectionsAnd
                                     .map(coll => cts.collectionQuery(coll));
                            }),
      andCollectionsQuery = andCollectionsArray
                            .map(item => cts.andQuery(item));
